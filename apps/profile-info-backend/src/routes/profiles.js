...
router.get('/:id', async (req, res) => {
  try {
    ...
    const prefsResult = await pool.query('SELECT key, value FROM preferences WHERE profile_id = $1', [id]);
...
router.post('/', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [
        user_id || null, name || null, email || null, avatar_url || null, bio || null,
        username || null, first_name || null, last_name || null, profile_picture_url || null,
        master_hub_id ?? null, master_country_id ?? null, primary_role || null, city || null, state || null,
      ]
    );
