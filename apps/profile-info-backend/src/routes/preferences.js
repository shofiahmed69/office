...
router.get('/', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT key, value FROM preferences WHERE profile_id = $1', [profileId]);
...
router.put('/', async (req, res) => {
  try {
    ...
    try {
      ...
      const { rows } = await client.query('SELECT key, value FROM preferences WHERE profile_id = $1', [profileId]);
